...
import science.aist.machinelearning.algorithm.gp.visualisation.jgraphx.GPVisualisation;
...
/**
 * UnitTestClass for {@link GPGeneCreator}.
 *
 * @author Daniel Wilfing
 * @since 1.0
 */
public class GPGeneCreatorTest {
    ...
    public void runTree() {
        ...
        //when
        GPGraphNode root = geneCreator.createGene(new ProblemGene<>(problem));
        Solution<Element[], Element[]> solution = (Solution) root.execute();

        //then
        //its possible that some trees don't return a solution
        if (solution != null) {
            evaluator.evaluateQuality(solution);

            Assert.assertNotNull(solution.getSolutionGenes().get(0).getGene());
            Assert.assertEquals(solution.getCachets().size(), 1);
            Assert.assertEquals(solution.getCachets().get(0).getQuality(), solution.getQuality());
        }
    }
    ...
    public void showVisualisation() {
        for (int i = 0; i < 100; i++) {
            ...
            GPVisualisation visualisation = new GPVisualisation();
            visualisation.drawGPGraph(root);

            GPTrim.trimGraph((ResultNode) root);

            GPVisualisation visualisation2 = new GPVisualisation();
            visualisation2.drawGPGraph(root);
